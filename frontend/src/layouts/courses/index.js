    ...
    action: (
      <MDBox display="flex" alignItems="center">
        <MDTypography
          ...
          color="text"
          fontWeight="medium"
          onClick={() => handleOpenEditModal(courseData)}
        >
          Edit
        </MDTypography>
        <MDTypography
          ...
          fontWeight="medium"
          sx={{ ml: 1 }}
          onClick={() => handleDeleteClick(courseData.id)}
        >
          Delete
        </MDTypography>
      </MDBox>
  ...
  const proceedDelete = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setCourseData((prevCourses) =>
          prevCourses.filter((course) => course.key !== selectedCourseId)
        );
    ...
    <DashboardLayout>
      ...
      <Dialog open={openConfirmDialog} onClose={cancelDelete}>
        ...
        <DialogActions>
          ...
          <Button onClick={proceedDelete} color="primary">
            Yes
          </Button>
        </DialogActions>
      </Dialog>
